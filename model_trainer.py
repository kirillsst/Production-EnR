...
class ModelTrain:
    ...
    @classmethod
    def load(cls, producer_type: str, features: List[str],
             target: str, save_dir: str = 'saved_models'):
        save_path = Path(save_dir)
        model_path = save_path / f"{producer_type}_random_forest_model.pkl"
        
        if not model_path.exists():
            raise FileNotFoundError(f"Modèle non trouvé")
        instance = cls(producer_type=producer_type, features=features, target=target, save_dir=save_dir)
        instance.model = joblib.load(model_path)
        
        print(f"Modèle charge pour {producer_type} (RandomForestRegressor).")
        return instance
